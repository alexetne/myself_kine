...
export class ProfileService {
  ...
  async get(userId: string): Promise<Record<string, unknown>> {
    const result = await this.database.query(
      "SELECT birth_year, practices, level, goal, weekly_availability, equipment, version, updated_at FROM sport_profile WHERE user_id = $1",
      [userId],
    );
    if (!result.rows[0])
      throw new NotFoundException({
        code: "PROFILE_NOT_FOUND",
        user_message: "Le profil sportif doit être complété.",
      });
  ...
  async upsert(
    ...
    if (!result.rows[0])
      throw new ConflictException({
        ...
        user_message: "Le profil a été modifié sur un autre appareil.",
      });
    return result.rows[0];
  }
}
